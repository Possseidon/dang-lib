...
enum class GCOption {
    ...
    CountBytes = LUA_GCCOUNTB, // Returns the remainder of dividing the current amount of bytes of memory in use by Lua by 1024.
    Step = LUA_GCSTEP, // gc(..., int stepsize); Performs an incremental step of garbage collection, corresponding to the allocation of stepsize Kbytes.
    IsRunning = LUA_GCISRUNNING, // Returns a boolean that tells whether the collector is running (i.e., not stopped).
    Incremental = LUA_GCINC, // gc(..., int pause, int stepmul, int stepsize); Changes the collector to incremental mode with the given parameters and returns the previous mode.
    Generational = LUA_GCGEN, // gc(..., int minormul, int majormul); Changes the collector to generational mode with the given parameters and returns the previous mode.
};
...
std::array<lua_CFunction, static_cast<std::size_t>(StandardLibrary::COUNT)> libraryFunctions = {
    luaopen_base,
    luaopen_coroutine,
    luaopen_table,
    luaopen_io,
...
std::array<const char*, static_cast<std::size_t>(StandardLibrary::COUNT)> libraryNames = {
    "_G",
    LUA_COLIBNAME,
    LUA_TABLIBNAME,
    LUA_IOLIBNAME,
...
std::array<const char*, static_cast<std::size_t>(LoadMode::COUNT)> loadModeNames = {
    nullptr,
    "b",
    "t",
    "bt"
